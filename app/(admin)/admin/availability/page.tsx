...
export default function AvailabilityPage() {
  ...
  const [selectedTimes, setSelectedTimes] = useState<Set<string>>(new Set(['09:00', '10:00', '11:00']))
    ...
    <div className="space-y-8 max-w-3xl">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Add open slots — visitors see only available times on the booking page.
        </p>
      </div>
      ...
      {tab === 'batch' && (
        <div className="rounded-xl border bg-card p-6 space-y-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Pick a date range, which days of the week, and what times — we'll generate all the slots at once.</p>
          </div>
          ...
          <div className="flex items-center gap-4 pt-2 border-t">
            <div className="flex-1">
              {batchPreview > 0 && (
                <p className="text-sm text-muted-foreground">
                  Will create <span className="font-semibold text-foreground">{batchPreview} slots</span>
                </p>
              )}
            ...
            <Button onClick={addBatch} disabled={batchAdding || batchPreview === 0}>
              {batchAdding
                ...
                : <><Zap className="h-4 w-4 mr-2" />Add {batchPreview > 0 ? batchPreview : ''} Slots</>}
            </Button>
          </div>
        </div>
      )}
      ...
      {tab === 'single' && (
        <div className="rounded-xl border bg-card p-6 space-y-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Pick a date and check off which times work.</p>
          </div>
      ...
      <div>
        ...
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground py-4 border rounded-xl text-center p-8">
            No upcoming slots yet — use the scheduler above to add some.
          </p>
        ) : (
          <div className="space-y-2">
            {upcoming.map(slot => (
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <Badge variant={slot.is_booked ? 'default' : 'secondary'} className="text-xs">
