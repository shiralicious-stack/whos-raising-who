    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-muted-foreground max-w-xs">
          A confirmation is on its way to <strong>{email}</strong>. Shira will send a video room link before your session.
        </p>
      </div>
    )
  }
          ...
          <div className="grid grid-cols-7 gap-y-1">
            {grid.map((day, i) => {
              ...
              return (
                <button
                  key={i}
                  onClick={() => available && !past && setSelectedDay({ y: viewYear, m: viewMonth, d: day })}
        ...
        {selectedSlot && (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="flex items-center justify-between p-3 rounded-xl bg-primary/5 border border-primary/20 mb-2">
              ...
              <button type="button" onClick={() => setSelectedSlot(null)} className="text-xs text-primary hover:underline">
                Change
              </button>
            </div>

            <div className="space-y-1.5">
              <Label htmlFor="c-name" className="text-xs">Name <span className="text-destructive">*</span></Label>
              <Input id="c-name" value={name} onChange={e => setName(e.target.value)} placeholder="Your full name" required className="h-9 text-sm" />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="c-email" className="text-xs">Email <span className="text-destructive">*</span></Label>
              <Input id="c-email" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" required className="h-9 text-sm" />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="c-notes" className="text-xs">What would you like to work on? <span className="text-muted-foreground">(optional)</span></Label>
              <Textarea id="c-notes" value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g. triggers, a specific situation, patterns I want to break..." rows={2} className="text-sm resize-none" />
            </div>
            ...
            <p className="text-xs text-center text-muted-foreground">You&apos;ll receive a confirmation email right away.</p>
          </form>
        )}
      </div>
    </div>
